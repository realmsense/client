...
void CDataPack::PackCell(int cell)
{
    InternalPack val;
...
void CDataPack::PackFunction(int function)
{
    InternalPack val;
...
void CDataPack::PackCellArray(int const* vals, int count)
{
    InternalPack val;
    val.type = CDataPackType::CellArray;

    val.pData.aval = new int[count + 1];
    memcpy(&val.pData.aval[1], vals, sizeof(int) * (count + 1));
    val.pData.aval[0] = count;
    elements.emplace(elements.begin() + position, val);
    position++;
}

void CDataPack::PackFloatArray(int const* vals, int count)
{
    InternalPack val;
...
int CDataPack::ReadFunction() const
{
...
int* CDataPack::ReadCellArray(int* size) const
{
    ...
    int* val = elements[position].pData.aval;
    int* ptr = &(val[1]);
    ++position;
...
int* CDataPack::ReadFloatArray(int* size) const
{
